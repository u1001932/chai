...
chai.use(chaiHttp);                   /** use the chai-http plugin **/  //!!tricky part


suite('Functional Tests', function() {

  // Mocha allows testing asyncronous operations.
  // There is a small (BIG) difference. Can you spot it ? => chai.use(chaiHttp)
  
  // ### EXAMPLE ### 
  test('Asynchronous test #example', function(done){ /** <= Pass a callback to the test function **/
    setTimeout(function(){
      assert.isOk('Async test !!');
      done(); /** Call 'done()' when the async operation is completed**/ //important!!
  ...
  suite('Integration tests with chai-http', function() {
    ...
    suite('GET /hello?name=[name] => "hello [name]"', function(){
      ...
      test('Test GET /hello with your name',  function(done){ // Don't forget the callback...
         chai.request(server)             // 'server' is the Express App
          .get('/hello?name=morris') /** <=== Put your name in the query **/ 
          .end(function(err, res){        // res is the response object
            ...
            assert.equal(res.status, 200);
             assert.equal(res.text, 'hello morris'/** <==  Put your name here **/);
            done();   // Always call the 'done()' callback when finished.
          });
      });
    ...
    suite('PUT /travellers', function(){
      ...
      test('send {surname: "da Verrazzano"}', function(done) {
        ...
        chai.request(server).put('/travellers').send({surname: "da Verrazzano"}).end(function(err, res){
          ...
          done(); //VERY IMPORTANT TO END WITH DONE() FOR ASYNCHRNOUS METHODS!!
